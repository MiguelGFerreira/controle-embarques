...
import { formatarData, getPaymentTerm } from './index';
...
export const gerarInvoice = async (data: Invoice[], showRFAText: boolean) => {
  ...
  const cnpj = data[0].FILIAL == 21 ? "27001247/0030-13" : "27001247/0037-90"
  const valorFormatado = new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(data.entries().reduce((acc, curr) => acc + curr[1].VALOR_INVOICE, 0));
            ...
            [
              { text: data.NUMERO_INVOICE, style: 'tableData', alignment: 'center', border: [true, true, true, true], }, // todas as bordas
              {
                stack: [
                  ...
                  { text: formatarData(new Date().toISOString(), true) },
                ],
                style: 'tableData',
                alignment: 'center',
